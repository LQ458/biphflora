    ...
    if(item.dbType !== "bird"){
      return <Card className="auth-card m-3">
        ...
        <div className="flex justify-content-end gap-2 mt-3">
          <Button
            label="Reject"
            icon="pi pi-times"
            onClick={() => handleEditDecision(item._id, false, item.dbType)}
            className="p-button-danger"
          />
          <Button
            label="Approve"
            icon="pi pi-check"
            onClick={() => handleEditDecision(item._id, true, item.dbType)}
      ...
      return <Card className="auth-card m-3">
        ...
        <div className="auth-content">
          ...
          <h3>Description 外貌特征: {item.appearance}</h3>
          <h3>Songs and Calls 鸣叫特征: {item.songs}</h3>
          <h3>Diet and Foraging 主要食谱与觅食行为: {item.diet}</h3>
          <h3>Habitat 栖息地: {item.habitat}</h3>
          <h3>Movements and Migration 迁徙行为: {item.migration}</h3>
          <h3>Breeding 繁育: {item.breeding}</h3>
          <h3>Juvenile Charateristics: {item.juvChar}</h3>
          <h3>Sub-adult Charateristics: {item.subChar}</h3>
          <h3>Male Adult Characteristics: {item.mAdultChar}</h3>
          <h3>Female Adult Charateristics: {item.fAdultChar}</h3>
          {/* <h3>Original Latin 繁育: {item.originalLatin}</h3> */}
          {/* <h3>Movements and Migration 迁徙行为: {item.migration}</h3> */}
      ...
      return <Card className="auth-card m-3">
        ...
        <div className="flex justify-content-end gap-2 mt-3">
          <Button
            label="Delete"
            icon="pi pi-trash"
            onClick={() => handleNewPostDecision(false, item._id, item.dbType)}
            className="p-button-danger"
          />
          <Button
            label="Submit"
            icon="pi pi-check"
            onClick={() => handleNewPostDecision(true, item._id, item.dbType)}
            className="p-button-success"
          />
        </div>
      </Card>
    ...
    <Card className="auth-card m-3">
      ...
      <div className="flex justify-content-end gap-2 mt-3">
        <Button
          label="Deny"
          icon="pi pi-times"
          onClick={() => handleNewCreationDecision(false, item._id, item.dbType)}
          className="p-button-danger"
        />
        <Button
          label="Approve"
          icon="pi pi-check"
          onClick={() => handleNewCreationDecision(true, item._id, item.dbType)}
          className="p-button-success"
        />
      </div>
    </Card>
