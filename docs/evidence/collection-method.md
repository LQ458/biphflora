...
When traffic aggregation is later approved, the report must retain the source
log on the server and save only aggregates. A candidate session is a sequence
...
5. Require a verified backup, dry-run manifest, and isolated staging sample
   before writing derivatives or changing path references.
