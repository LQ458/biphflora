...
| Current deployed code commit          | c5e348e                                                                   | Direct                  | CSS-only style fix; server marker, released CSS hash, public checks, and browser comparison agree                |
| Current deployment date               | 2026-07-23                                                                | Direct for this cutover | Asset-only CSS/HTML cutover; historical deployment count remains unavailable                                    |
| Continuous service duration           | Unavailable                                                               | Unavailable             | No availability monitor or complete incident history                                                         |
| Interrupted-service periods           | At least one automatic backend reload observed during the 2026-07-23 guarded style cutover attempt; historical periods unavailable | Partial | Exact interruption duration and public impact were not instrumented                                           |
...
| Production/staging/local relationship | Production style release c5e348e; local branch contains its acceptance report; no independent staging found   | Direct/partial          | Isolated release checks and live browser comparison passed; staging parity and restore rehearsal remain unavailable |
...
| Media filesystem population                            | 1,033 readable originals / 2,109,916,481 bytes; 3,099 responsive derivatives / 237,052,126 bytes | Direct            | Full original and responsive-derivative population; files are not proven unique photographs |
...
| Metric                                              | Current value                                                                                 | Status         | Source and limitation                                                                                 |
| --------------------------------------------------- | --------------------------------------------------------------------------------------------- | -------------- | ----------------------------------------------------------------------------------------------------- |
| Point-in-time public HTTPS                          | HTTP 200                                                                                      | Direct         | Observed for root, JSON readiness, the 152-record approved catalogue, a detail read, and responsive WebP through `/api` |
...
| Automated restart/health/alerting                   | Liveness/readiness endpoints exist; historical monitor coverage unavailable                   | Direct/partial | MongoDB and Redis readiness passed after the controlled release restart; external monitoring and alert coverage are not verified |
...
| Test coverage/CI/security scan                      | 39 backend and 19 frontend tests pass with a production build; CI workflow present; numeric coverage unavailable | Partial        | Production dependency audits report zero known issues; development trees and hosted CI history retain limitations documented in phase 12 |
